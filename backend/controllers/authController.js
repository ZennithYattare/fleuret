...
exports.login = catchAsyncErrors(async (req, res, next) => {
	...
	const userUsername = await User.findOne({ username: email }).select(
		"+password"
	);
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
	...
	if (!user) {
		return next(
			new ErrorHandler("Password reset link is invalid or has expired")
		);
	}
...
exports.registerCustomer = catchAsyncErrors(async (req, res, next) => {
	...
	if (userEmail || userUsername) {
		return next(
			new ErrorHandler("Email account or username already exists", 404)
		);
	}
	if (passVal.validate(req.body.password) !== true) {
		return next(new ErrorHandler("Please follow password format", 400));
	...
	const registerToken = jwt.sign(
		...
		{ expiresIn: process.env.REGISTER_EXPIRES }
	);
			...
			User.findOne({ email }).exec((err, existingUser) => {
				...
				const user = User.create(customer).then((data) => {
					...
					res.status(201).json({
						success: true,
						message:
							"Congratulations! Your FleuretPH/TK account has been successfully registered. You may now log in.",
					});
				});
			});
		});
...
exports.deactivate = catchAsyncErrors(async (req, res, next) => {
	...
	if (orders.length > 0) {
		...
		console.log("here");
		return next(
			new ErrorHandler(`User has existing orders. Cannot deactivate.`)
		);
	}
...
exports.getDeactivatedUsers = catchAsyncErrors(async (req, res, next) => {
	const users = await User.find({ isDeactivated: true }).sort({
		role: 1,
		first_name: 1,
	});
